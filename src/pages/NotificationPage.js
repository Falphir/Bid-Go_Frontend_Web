...
export default function NotificationPage() {
    ...
    if (meLoading || loading) return <StatusMessage type="loading">Loading notifications...</StatusMessage>;
        ...
        <div className="notif-wrapper">
            <div className="notif-card-container">
                ...
                <div className="notif-list">
                    {filtered.length === 0 && <p className="notif-empty-page">No notifications found.</p>}

                    {filtered.map((n) => (
                        <NotificationCard key={n.notificationId} notification={n} onMarkRead={(id) => markRead(id)} />
                    ))}
                </div>
            </div>
        </div>
